...
class PubSubSettingsController extends GetxController {
  ...
  Iterable<DroppedDeviceModel> get devices => workspace.dropped
      .where((element) => element.model.type == DeviceType.ied);
  @override
  void onInit() {
    super.onInit();
  ...
  void toggleCross(
      DroppedDeviceModel device, DroppedDeviceModel another, int i, int j) {
    if (!data.containsKey(device)) data[device] = {};
    if (!data[device].containsKey(another)) data[device][another] = {};
    final key = '$i-$j';
    if (data[device][another].remove(key) != true)
      data[device][another][key] = true;
    update();
  }
